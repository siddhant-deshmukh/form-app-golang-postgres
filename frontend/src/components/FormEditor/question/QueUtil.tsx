...
export const ansTypesStates = {
  ...
  const SelectQueTypeList = ({ ansType, changeAnsType }: { ansType: IQuestion['ques_type'], changeAnsType: (prevType: IQuestion['ques_type'], selectedType: IQuestion['ques_type']) => void }) => {
  
    const typesArr = ['short', 'long', 'mcq', 'checkbox', 'dropdown']
    return (
      <div className={`absolute w-44 z-30 flex flex-col bg-white border border-gray-300`}>
        {
          typesArr.map((type) => {
            return (
              <button
                key={type}
                onClick={(event) => { event.preventDefault(); changeAnsType(ansType, type as IQuestion['ques_type']) }}
