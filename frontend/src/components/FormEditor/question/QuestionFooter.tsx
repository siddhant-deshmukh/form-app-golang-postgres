...
import { editQue, addQue, deleteQue, saveQue } from '../../../features/queState'
import { createNewQuestion, deleteQuestion, updateQuestionChanges } from '../../../utils/questionController'
...
function QuestionFooter(
  ...
  const addQuestion = async () => {
    let ind = que_seq.indexOf(queKey)
    ind = (ind < 0) ? 0 : ind
    ind = ind + 1
    const { status, que } = await createNewQuestion(question, ind)
    if (status === 201 && que) {
      dispatch(addQue({ index: ind, question: que }));
    } else {
  ...
  const saveQuestion = async () => {
    ...
    if (status !== 200) {
      dispatch(setMsg({ msgType: "err", msg: `Something went wrong while saving the updates (${status})` }))
    } else {
      dispatch(saveQue(queKey))
    }
    setLoading(false)
  }
  const delQue = async () => {
    ...
    <div className='px-4 mt-4'>
      <div className='w-full py-1  border-t border-t-gray-300 mt-2 flex  h-fit items-center space-x-6'>
          ...
          <div className='w-fit'>
            {
              ...
              <>
                {
                  (question.correct_ans === undefined || question.correct_ans === null) &&
                  <button
                ...
                }
                {
                  !(question.correct_ans === undefined || question.correct_ans === null) &&
                  <button
                    className='flex space-x-2text text-blue-500 font-medium text-sm items-center'
                    onClick={(event) => {
                      event.preventDefault();
                      dispatch(editQue({
                        id: queKey,
        ...
        <button
          ...
          className={`w-28  hidden sm:block ${(question.isSaved) ? "bg-blue-100 text-gray-600" : "bg-blue-500 text-white"} rounded-md p-2 text-xs `}
          onClick={(event) => {
            event.preventDefault();
            saveQuestion()
          }}
      ...
      <button
        disabled={(question.isSaved)}
        // ${((question._id?.slice(0, 3) === 'new') || (question.savedChanges)) ? "bg-blue-100 text-gray-600" : "bg-blue-500 text-white"}
        className={`w-fit ml-auto mt-3 block sm:hidden  rounded-md p-2 text-xs `}
        onClick={(event) => {
          event.preventDefault();
          saveQuestion()
        }}
